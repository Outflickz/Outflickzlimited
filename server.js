...
function generateOrderEmailHtml(order) {
    ...
    const subtotal = KoboToNgn(subtotalKobo);
    const shipping = KoboToNgn(shippingFeeKobo);
    const tax = KoboToNgn(taxKobo);
...
function generateOrderEmailHtml(order) {
    // --- START: Item HTML Generation (Focus of Redesign) ---
    const itemsHtml = order.items.map(item => {
        // Use a larger placeholder if the image URL is missing
        const itemImageUrl = item.imageUrl || 'https://placehold.co/100x100/f8f8f8/999999?text=NO+IMG';
        
        return `
            <tr>
                <!-- Product Details Column - Increased size and changed object-fit for full display -->
                <td style="padding: 12px; border: 1px solid #ddd; display: flex; align-items: flex-start;">
                    <img 
                        src="${itemImageUrl}" 
                        alt="${item.name}" 
                        style="width: 100px; height: 100px; object-fit: contain; margin-right: 20px; border-radius: 8px; border: 1px solid #D1D5DB; display: block;"
                    >
                    <div>
                        <p style="margin: 0; font-weight: bold; font-size: 1.1em; color: #1F2937;">${item.name}</p>
                        <p style="margin: 5px 0 0 0; font-size: 0.95em; color: #4B5563;">
                            Size: ${item.size || 'N/A'} 
                            ${item.variation ? `| Details: ${item.variation}` : ''}
                        </p>
                        <p style="margin: 2px 0 0 0; font-size: 0.9em; color: #4B5563;">
                            Price per unit: ${formatCurrency(KoboToNgn(item.priceAtTimeOfPurchase || 0))}
                        </p>
                    </div>
                </td>
                <!-- Quantity Column -->
                ...
                <!-- Total Price Column -->
                <td style="padding: 8px; border: 1px solid #ddd; text-align: right; font-weight: bold; color: #1F2937;">
                    ${formatCurrency(KoboToNgn((item.priceAtTimeOfPurchase || 0) * item.quantity))}
                </td>
            </tr>
        `;
    }).join('');
    // --- END: Item HTML Generation ---

    // Calculate totals (fallbacks added for robustness)
    const totalAmountKobo = order.totalAmount || order.amountPaidKobo || 0;
    const shippingFeeKobo = order.shippingFee || 0;
    const taxKobo = order.tax || 0; 
    // Calculate subtotal by subtracting known non-subtotal components
    const subtotalKobo = totalAmountKobo - shippingFeeKobo - taxKobo;
    ...
    return `
        <div style="max-width: 700px; margin: 0 auto; padding: 25px; border: 1px solid #E5E7EB; border-radius: 12px; font-family: 'Inter', Arial, sans-serif; line-height: 1.6; color: #374151; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
            <h2 style="color: #4F46E5; border-bottom: 3px solid #4F46E5; padding-bottom: 15px; margin-top: 0; font-size: 1.8em;">
                Order Confirmed! 
                <span style="float: right; font-size: 0.8em; color: #1F2937;">#${order.orderReference || order._id}</span>
            </h2>
            <p style="font-size: 1.1em;">Hello ${address.firstName || 'Customer'},</p>
            <p>Your order has been successfully confirmed and is now being prepared for shipping. Thank you for shopping with us!</p>

            <h3 style="margin-top: 30px; color: #1F2937; border-bottom: 1px solid #ddd; padding-bottom: 5px;">Order Summary</h3>
            <table style="width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 1em;">
                <thead>
                    <tr style="background-color: #F3F4F6;">
                        <th style="padding: 12px; border: 1px solid #ddd; text-align: left; width: 65%;">Product Details</th>
                        <th style="padding: 12px; border: 1px solid #ddd; width: 10%; text-align: center;">Qty</th>
                        <th style="padding: 12px; border: 1px solid #ddd; width: 25%; text-align: right;">Line Total</th>
                    </tr>
                </thead>
                <tbody>
                    ${itemsHtml}
            ...
            <div style="margin-top: 25px; text-align: right;">
                <table style="width: 50%; margin-left: auto; border-collapse: collapse; font-size: 1.1em;">
                    <tr><td style="padding: 5px 0;">Subtotal:</td><td style="text-align: right; color: #4B5563;">${formatCurrency(subtotal)}</td></tr>
                    <tr><td style="padding: 5px 0;">Shipping:</td><td style="text-align: right; color: #4B5563;">${formatCurrency(shipping)}</td></tr>
                    <tr><td style="padding: 5px 0;">Tax:</td><td style="text-align: right; color: #4B5563;">${formatCurrency(tax)}</td></tr>
                    <tr>
                        <td style="padding: 12px 0; border-top: 2px solid #333; font-weight: bold; font-size: 1.2em;">Order Total:</td>
                        <td style="text-align: right; font-weight: bold; border-top: 2px solid #333; color: #4F46E5; font-size: 1.2em;">${formatCurrency(finalTotal)}</td>
                    </tr>
                </table>
            </div>
            ...
            <div style="padding: 15px; background-color: #F9FAFB; border-radius: 8px; border: 1px solid #E5E7EB;">
                <p style="margin: 0 0 5px 0;"><strong>Recipient:</strong> ${address.firstName} ${address.lastName}</p>
                <p style="margin: 0 0 5px 0;"><strong>Address:</strong> ${fullAddress}</p>
                <p style="margin: 0 0 5px 0;"><strong>Email:</strong> ${address.email || 'N/A'}</p>
                <p style="margin: 0 0 5px 0;"><strong>Phone:</strong> ${phoneNumber}</p>
                <p style="margin: 0;"><strong>Status:</strong> <span style="font-weight: bold; color: #059669;">${order.status || 'Pending Shipment'}</span></p>
            </div>

            <p style="margin-top: 40px; text-align: center; font-size: 0.9em; color: #6B7280;">
                If you have any questions, please reply to this email or contact our support team.
            </p>
            <p style="text-align: center; font-size: 0.8em; color: #9CA3AF;">
