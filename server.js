...
// =========================================================
// 12. GET /api/admin/inventory/available-stock - Fetch Current Available Stock (Admin Protected)
// =========================================================
app.get('/api/admin/inventory/available-stock', verifyToken, async (req, res) => {
    try {
        // Ensure you have access to the Mongoose models (assumes they are defined elsewhere)
        const WearsModel = mongoose.models.WearsCollection || mongoose.model('WearsCollection');
        const CapModel = mongoose.models.CapCollection || mongoose.model('CapCollection');
        const NewArrivalsModel = mongoose.models.NewArrivals || mongoose.model('NewArrivals');
        const PreOrderModel = mongoose.models.PreOrderCollection || mongoose.model('PreOrderCollection');
        
        // Get the category filter from the query parameter (e.g., ?category=wears)
        const categoryFilter = req.query.category ? req.query.category.toLowerCase() : 'all';

        // 1. Define the models to query based on the filter
        const modelsToQuery = [];

        // Define a function to process each product model
        const processModel = (model, categoryName) => {
            // A common projection to standardize the output structure across all collections
            const pipeline = [
                // Match items that are currently in stock (quantity > 0)
                { $match: { quantity: { $gt: 0 } } }, 
                { $project: {
                    _id: 0,
                    productId: '$_id', // Assuming _id is the unique product ID/SKU for the item/variant
                    name: '$name',
                    variant: { 
                        // Assuming size and color fields exist on the product documents
                        $concat: [
                            { $ifNull: ['$size', 'N/A'] }, 
                            '/', 
                            { $ifNull: ['$color', 'N/A'] }
                    ...
                    quantity: '$quantity', // The current stock count
                    category: categoryName, // Manually assign the clean category name
                    lastUpdate: '$updatedAt' // Use the document update time
                }}
            ];

            // If the category filter is 'all', return the promise to fetch from this model
            if (categoryFilter === 'all' || categoryFilter === categoryName.toLowerCase().replace('-', '')) {
                modelsToQuery.push(model.aggregate(pipeline));
            }
        };

        // 2. Queue the aggregation promises for each relevant model
        processModel(WearsModel, 'Wears');
        processModel(CapModel, 'Caps');
        processModel(NewArrivalsModel, 'New Arrivals');
        processModel(PreOrderModel, 'Pre-Orders');

        // Handle case where an invalid category is provided and no models are queued
        if (modelsToQuery.length === 0 && categoryFilter !== 'all') {
             return res.status(400).json({ message: 'Invalid category filter provided or no stock available for that category.' });
        }


        // 3. Execute all aggregation queries concurrently
        const results = await Promise.all(modelsToQuery);
        
        // 4. Flatten the results array (array of arrays -> single array)
        const availableStock = results.flat(); 

        // 5. Sort the final list (e.g., by name or latest update)
        ...
        res.status(500).json({ message: 'Failed to retrieve available stock inventory.' });
    }
});
