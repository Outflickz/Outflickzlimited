...
case 'delete-tracksuit': {
    ...
    if (!email || !masterKey || !newPassword) {
        return { statusCode: 400, headers, body: JSON.stringify({ message: "All security parameters required" }) };
    }
    if (!MASTER_ACCESS_KEY || masterKey.trim() !== MASTER_ACCESS_KEY) {
...
case 'verify-payment': {
    ...
    try {
        ...
        if (paymentStatus.status && paymentStatus.data.status === 'success') {
            ...
            const taxNaira = subtotalNaira * 0.009;
